...
const AdvancedAudioPlayer: React.FC<AdvancedAudioPlayerProps> = memo(({
  ...
  const { isStreaming, streamingMessageId, streamingText } = useStreamingStore();

  if (!currentMessageId && !isLoading && !isPlaying && !currentPlayingText && !isStreaming) {
    return null;
  }

  const isStreamingHere = isStreaming && streamingMessageId && currentMessageId?.startsWith(streamingMessageId);
  const displayMessageText = isStreamingHere ? streamingText : (currentMessageText || currentPlayingText || t.audioPlayback);
